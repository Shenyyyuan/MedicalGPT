...
class DataArguments:
    ...
    general_train_file_dir: Optional[str] = field (
        default=None,
        metadata={"help": "Optional general-domain jsonl folder to mix into SFT training."},
    )
    general_dataset_name: Optional[str] = field (
        default=None,
        metadata={"help": "Optional HuggingFace dataset name for general-domain SFT mixing."},
    )
    general_dataset_config_name: Optional[str] = field (
        default=None,
        metadata={"help": "Optional HuggingFace dataset config for general-domain SFT mixing."},
    )
    general_dataset_split: Optional[str] = field (
        default="train",
        metadata={"help": "HuggingFace split to load for general-domain SFT mixing."},
    )
    mixed_train_samples: Optional[int] = field (
        default=None,
        metadata={"help": "Total mixed SFT train samples. Example: 20000 for 80/20 mixed training."},
    )
    domain_sample_ratio: float = field (
        default=1.0,
        metadata={"help": "Domain sample ratio when mixing general data, e.g. 0.8."},
    )
    ...
    def __post_init__(self):
        ...
        if not 0 < self.domain_sample_ratio <= 1:
            raise ValueError ("domain_sample_ratio must be in (0, 1].")
        if self.mixed_train_samples is not None and self.mixed_train_samples == 0:
            raise ValueError ("mixed_train_samples must be positive or -1.")
...
def iter_sft_sources(examples):
    """Yield conversation-style samples from supported SFT jsonl formats."""
    row_count = 0
    ...
    if row_count == 0:
        raise KeyError (
            "SFT 数据缺少 conversations，且没有 instruction/output、prompt/chosen、"
            "questions/answers 或 question/answer 字段。"
        )
    ...
    for i in range (row_count):
        source = get_value ("conversations", i)
        if source:
            yield source
            continue
        ...
        for prompt_name, response_name in pairs:
            prompt = get_value (prompt_name, i)
            response = get_value (response_name, i)
...
def sample_dataset(dataset, sample_count: Optional[int], seed: int):
    """Shuffle and select up to sample_count rows without replacement."""
    shuffled = dataset.shuffle (seed=seed)
    if sample_count is None or sample_count < 0 or sample_count >= len (shuffled):
        return shuffled
    return shuffled.select (range (sample_count))


def build_mixed_train_dataset(domain_dataset, general_dataset, total_samples, domain_ratio, seed):
    """Build a shuffled domain/general mix, capped by available rows."""
    if not 0 < domain_ratio < 1:
        raise ValueError ("domain_ratio must be in (0, 1) when general data is mixed.")

    if total_samples is None or total_samples < 0:
        domain_count = len (domain_dataset)
        general_count = round (domain_count * (1 - domain_ratio) / domain_ratio)
    else:
        domain_count = round (total_samples * domain_ratio)
        general_count = total_samples - domain_count

    domain_count = min (domain_count, len (domain_dataset))
    general_count = min (general_count, len (general_dataset))

    mixed_parts = [
        sample_dataset (domain_dataset, domain_count, seed),
        sample_dataset (general_dataset, general_count, seed + 1),
    ]
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if "validation" not in raw_datasets.keys ():
            ...
            raw_datasets["train"] = split["train"]
            raw_datasets["validation"] = split["test"]
    general_train_dataset = None
    if data_args.general_train_file_dir is not None and os.path.exists (data_args.general_train_file_dir):
        general_train_files = glob (f'{data_args.general_train_file_dir}/**/*.jsonl', recursive=True)
        logger.info (f"general train files: {general_train_files}")
        general_train_dataset = load_local_json_datasets (
            {"train": general_train_files},
            cache_dir=model_args.cache_dir,
        )["train"]
    elif data_args.general_dataset_name is not None:
        logger.info (
            f"Loading general dataset: {data_args.general_dataset_name}, "
            f"config={data_args.general_dataset_config_name}, split={data_args.general_dataset_split}"
        )
        general_train_dataset = load_dataset (
            data_args.general_dataset_name,
            data_args.general_dataset_config_name,
            split=data_args.general_dataset_split,
            cache_dir=model_args.cache_dir,
        )

    if general_train_dataset is not None:
        raw_datasets["train"] = build_mixed_train_dataset (
            raw_datasets["train"],
            general_train_dataset,
            data_args.mixed_train_samples,
            data_args.domain_sample_ratio,
            training_args.seed,
        )
        logger.info (
            "Mixed train dataset: "
            f"domain_ratio={data_args.domain_sample_ratio}, "
            f"mixed_train_samples={data_args.mixed_train_samples}, "
            f"final_train_rows={len (raw_datasets['train'])}"
        )
    ...
    def preprocess_function(examples):
        ...
        def get_dialog(examples):
            ...
            for i, source in enumerate (iter_sft_sources (examples)):
                system_prompt = ""
                tools_text = ""
                        ...
                        if role == "function_call":
                            ...
                            if "name" in fc_dict and "arguments" in fc_dict:
                                if script_args.tool_format:
                                    tu = get_tool_utils (script_args.tool_format)
                                    value = tu.function_formatter ([FunctionCall (fc_dict["name"],
                                                                                  json.dumps (fc_dict["arguments"],
                                                                                              ensure_ascii=False))])
                                else:
