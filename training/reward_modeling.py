...
def sync_model_special_token_ids(model, tokenizer):
    ...
    for module in model.modules():
        objects.append(getattr(module, "config", None))
        objects.append(getattr(module, "generation_config", None))
    for obj in objects:
        if obj is None:
            continue
        nested_objects = [obj, getattr(obj, "text_config", None)]
...
def main():
    ...
    # 显式设置每个进程对应的 GPU（解决 NCCL duplicate GPU 错误）
    ...
    sync_model_special_token_ids(trainer.model, tokenizer)
